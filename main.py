...
# ============================================================
# ===== ASCII BANNERS (Only for start & about) =====
# ============================================================
BANNER_ARMADA = """
┌─────────────────────────────────────────────────────┐
│  █████╗ ██████╗ ███╗   ███╗ █████╗ ██████╗  █████╗ │
...
def make_box(title, lines):
    """Create a clean terminal box with title."""
...
async def binary_upload_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied – only admins can deploy binaries.")
            return ConversationHandler.END
        token_manager.health_check()
        if not github_tokens:
            await update.message.reply_text("❌ Token Vault Empty – add tokens via /inject or /addtoken")
            return ConversationHandler.END
...
async def binary_upload_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        token_manager.health_check()
        if not github_tokens:
            await update.message.reply_text("❌ No valid tokens. Add with /inject or /addtoken")
            return ConversationHandler.END
...
# ============================================================
# ===== STRIKE COMMAND (with aliases /attack) =====
# ============================================================
async def strike_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        token_manager.health_check()
        if not github_tokens:
            await update.message.reply_text("❌ No GitHub tokens. Add with /inject or /addtoken")
            return

        healthy = token_manager.get_healthy_tokens(MAX_REPOS_PER_ATTACK)
        if not healthy:
            await update.message.reply_text("❌ No healthy tokens available. Check /scan or /checktokens")
            return
...
async def abort_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not can_attack(user_id):
            ...
            return
        if not active_attacks:
            await update.message.reply_text("No active raids to abort.")
            return
        count = len(active_attacks)
        for aid in list(active_attacks.keys()):
            finish_attack(aid)
        await update.message.reply_text(f"🛑 Aborted {count} strike(s). System idle.")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")

# ============================================================
# ===== TOKEN COMMANDS (All original aliases) =====
# ============================================================
async def inject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        user_id = update.effective_user.id
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied – only admins can inject tokens.")
            return
        if len(context.args) != 1:
            await update.message.reply_text("Usage: /inject <github_token> (or /addtoken)")
            return
        ...
        for t in github_tokens:
            if t.get('username') == username:
                await update.message.reply_text(f"⚠️ @{username} already has a token. Remove first.")
                return
        ...
        save_json('github_tokens.json', github_tokens)
        token_manager.health_check()
        await update.message.reply_text(f"✅ Token injected for @{username} (Repo: {repo_name})")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:200]}")
...
async def vault_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = ["🔐 TOKEN VAULT", "─────────────────────────────"]
        if removed > 0:
            lines.append(f"🧹 Removed {removed} expired tokens")
        for i, t in enumerate(github_tokens, 1):
            ...
            lines.append(f"{i}. @{username} – {token_short} (rem: {remaining})")
...
async def scan_tokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_owner(user_id):
            ...
            return
        if not github_tokens:
            await update.message.reply_text("No tokens to scan.")
            return
        removed = token_manager.health_check()
        lines = ["🔍 TOKEN HEALTH SCAN", "─────────────────────────────"]
        lines.append(f"Total: {len(github_tokens)}")
        if removed > 0:
        ...
        for i, t in enumerate(github_tokens, 1):
            ...
            token_short = t['token'][:10] + "…" + t['token'][-4:]
            lines.append(f"{i}. {status} @{username} – {token_short} (rem: {remaining})")
        box = make_box("🔍 TOKEN HEALTH", lines)
        await update.message.reply_text(f"<pre>{box}</pre>", parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")

async def eject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # /eject or /removetoken
    try:
        user_id = update.effective_user.id
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied")
            return
        if len(context.args) != 1:
            await update.message.reply_text("Usage: /eject <token> (or /removetoken)")
            return
        ...
        for i, t in enumerate(github_tokens):
            if t.get('token') == token:
                ...
                break
        if found:
            await update.message.reply_text(f"✅ Token ejected. Remaining: {len(github_tokens)}")
        else:
...
async def purge_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(context.args) == 1 and context.args[0].lower() == "confirm":
            ...
            save_json('github_tokens.json', github_tokens)
            token_manager.health_check()
            await update.message.reply_text(f"🗑️ Purged {count} tokens.")
        else:
            await update.message.reply_text(f"⚠️ Delete ALL {count} tokens? Use /purge confirm (or /cleartokens confirm)")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
async def grant_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(context.args) != 2:
            await update.message.reply_text("Usage: /grant <user_id> <days> (or /approve)")
            return
        ...
        try:
            await context.bot.send_message(target_id, "✅ Access Granted! Use /start to launch.")
        except:
            pass
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
async def revoke_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_owner(user_id):
            ...
            return
        if len(context.args) != 1:
            await update.message.reply_text("Usage: /revoke <user_id> (or /remove)")
            return
        target_id = int(context.args[0])
...
async def list_users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for uid, data in approved_users.items():
            lines.append(f"`{uid}` – {data.get('days', '?')}d")
        box = make_box("👥 USERS", lines)
        await update.message.reply_text(f"<pre>{box}</pre>", parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
async def requests_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for u in pending_users:
            lines.append(f"`{u['user_id']}` – @{html.escape(u['username'])}")
        box = make_box("⏳ PENDING", lines)
        await update.message.reply_text(f"<pre>{box}</pre>", parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")
...
async def announce_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_owner(user_id):
            ...
            return
        if not context.args:
            await update.message.reply_text("Usage: /announce <message> (or /broadcast)")
            return
...
async def shield_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_owner(user_id):
            ...
            return
        if len(context.args) != 1:
            await update.message.reply_text("Usage: /shield <on/off> (or /maintenance)")
            return
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "<b>🤖 ARMADA – FULL COMMAND REFERENCE</b>\n\n"
        "<b>Strike</b>\n"
        "/strike or /attack – Launch strike\n"
        "/payload or /status – Live feed\n"
        "/abort or /stop – Abort all\n\n"
        "<b>Token Management</b>\n"
        "/inject or /addtoken – Add token\n"
        "/eject or /removetoken – Remove token\n"
        "/scan or /checktokens – Health check\n"
        "/purge or /cleartokens – Wipe vault\n"
        "/vault or /tokens – List tokens\n\n"
        "<b>User Management</b>\n"
        "/grant or /approve – Grant access\n"
        "/revoke or /remove – Revoke access\n"
        "/list or /users – List approved\n"
        "/requests or /pending – Pending\n"
        "/announce or /broadcast – Broadcast\n"
        "/shield or /maintenance – Maintenance\n\n"
        "<b>Binary Deploy</b>\n"
        "/deploy or /binary_upload – Upload spider\n\n"
        "<b>Utilities</b>\n"
        "/start – Dashboard\n"
        "/myid – Your ID\n"
        "/about – Bot info\n"
...
async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"<b>{BANNER_ARMADA}</b>\n\n"
        "🤖 ARMADA v3.0 (Full Commands Restored)\n"
...
async def error_handler(update, context):
    ...
    if update and update.effective_message:
        try:
            await update.effective_message.reply_text("⚠️ System error. Check logs.")
        except:
            pass
...
def main():
    try:
        ...
        # Binary upload conversation (supports /deploy and /binary_upload)
        ...
        # ===== ALL COMMANDS (Full original set) =====
        # Core
        app.add_handler(CommandHandler("start", start_cmd))
        app.add_handler(CommandHandler("strike", strike_cmd))
        app.add_handler(CommandHandler("attack", strike_cmd))  # alias
        app.add_handler(CommandHandler("payload", payload_cmd))
        app.add_handler(CommandHandler("status", payload_cmd))  # alias
        app.add_handler(CommandHandler("abort", abort_cmd))
        app.add_handler(CommandHandler("stop", abort_cmd))  # alias
        ...
        app.add_handler(CommandHandler("addtoken", inject_cmd))  # alias
        app.add_handler(CommandHandler("eject", eject_cmd))
        app.add_handler(CommandHandler("removetoken", eject_cmd))  # alias
        app.add_handler(CommandHandler("purge", purge_cmd))
        app.add_handler(CommandHandler("cleartokens", purge_cmd))  # alias
        app.add_handler(CommandHandler("scan", scan_tokens_cmd))
        app.add_handler(CommandHandler("checktokens", scan_tokens_cmd))  # alias
        app.add_handler(CommandHandler("vault", vault_cmd))
        app.add_handler(CommandHandler("tokens", vault_cmd))  # alias

        # User admin
        app.add_handler(CommandHandler("grant", grant_cmd))
        app.add_handler(CommandHandler("approve", grant_cmd))  # alias
        app.add_handler(CommandHandler("revoke", revoke_cmd))
        app.add_handler(CommandHandler("remove", revoke_cmd))  # alias
        app.add_handler(CommandHandler("list", list_users_cmd))
        app.add_handler(CommandHandler("users", list_users_cmd))  # alias
        app.add_handler(CommandHandler("requests", requests_cmd))
        app.add_handler(CommandHandler("pending", requests_cmd))  # alias
        app.add_handler(CommandHandler("announce", announce_cmd))
        app.add_handler(CommandHandler("broadcast", announce_cmd))  # alias
        app.add_handler(CommandHandler("shield", shield_cmd))
        app.add_handler(CommandHandler("maintenance", shield_cmd))  # alias

        app.add_handler(CallbackQueryHandler(button_callback))
        app.add_error_handler(error_handler)

        logger.info("🚀 ARMADA ONLINE – ALL COMMANDS RESTORED + CLEAN DESIGN")
