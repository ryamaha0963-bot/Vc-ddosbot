...
pending_users = {}
attack_counters = {}
token_health = {}          # cache for health check results
...
def validate_github_token(token, check_rate=True):
    """
    Returns: (is_valid, message, rate_info)
    rate_info: dict with 'remaining' and 'reset' timestamps, or None.
    """
    try:
...
def auto_remove_expired(dry_run=False):
    """Remove invalid tokens. If dry_run=True, just count without deleting."""
    ...
    for td in github_tokens:
        ...
        if not token:
            removed += 1
            continue
        # Check validity, but don't fail on rate limit (we only remove on permanent errors)
    ...
    if removed > 0 and not dry_run:
        ...
        logger.info(f"🧹 Dry run: {removed} invalid tokens would be removed.")
    return removed
...
async def addtoken_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        is_valid, info, _ = validate_github_token(token)  # rate check is on by default
        if not is_valid:
...
async def tokenhealth_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show health of all tokens without removing any."""
    try:
        user_id = update.effective_user.id
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied", parse_mode='HTML')
...
"""
                ...
                # Permanently remove only on 401/404 (invalid token or repo)
                if e.status in [401, 404]:
                    ...
                    # We continue loop without increment because we removed an element
                continue
            except Exception as e:
                last_error = e
...
async def about_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Health      :  /tokenhealth for live status\n"
...
def main():
    try:
        ...
        app.add_handler(CommandHandler("tokenhealth", tokenhealth_cmd))   # NEW
        ...
        logger.info("⚡ STRIKE ENGINE v3.2 is ONLINE (Power + Rotation + Health)")
