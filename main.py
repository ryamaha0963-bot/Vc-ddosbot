...
# ============================================================
def format_banner(banner_text, title=None):
    lines = banner_text.split('\n')
    if title:
        lines.insert(1, f"│  {title.center(50)}  │")
    return '\n'.join(lines)
...
# ============================================================
# ===== BINARY UPLOAD (no style changes) =====
# ============================================================
...
async def binary_upload_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        token_manager.health_check()
        if not github_tokens:
            await update.message.reply_text("❌ No valid tokens. Add with /inject", parse_mode='HTML')
            return ConversationHandler.END
...
async def inject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(context.args) != 1:
            await update.message.reply_text("📖 <b>Usage:</b> <code>/inject &lt;github_token&gt;</code>", parse_mode='HTML')
            return
        ...
        for t in github_tokens:
            if t.get('username') == username:
                await update.message.reply_text(
                    ...
                    f"Remove it first with <code>/eject</code>.",
                    parse_mode='HTML'
                )
                return
...
async def scan_tokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/scan – Health check with rate‑limit info."""
    try:
        user_id = update.effective_user.id
        if not is_owner(user_id):
            await update.message.reply_text("⛔ Access Denied", parse_mode='HTML')
...
async def eject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_owner(user_id):
            ...
            return
        if len(context.args) != 1:
            await update.message.reply_text("📖 Usage: <code>/eject &lt;token&gt;</code>", parse_mode='HTML')
            return
...
async def purge_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(context.args) == 1 and context.args[0].lower() == "confirm":
            ...
            await update.message.reply_text(f"⚠️ Delete ALL {count} tokens? Use <code>/purge confirm</code>", parse_mode='HTML')
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}")

# ============================================================
# ===== STRIKE COMMAND (Hacker Style) =====
# ============================================================
async def strike_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/strike <ip> <port> <time> – Launch a multi‑repo attack."""
    try:
        user_id = update.effective_user.id
        ...
        if len(context.args) != 3:
            await update.message.reply_text(
                "📖 <b>Usage:</b> <code>/strike &lt;ip&gt; &lt;port&gt; &lt;time&gt;</code>\n\n"
...
"""
        ...
        # Build professional hacker output
        ...
        msg += f"\n🛑 <b>Abort:</b> Use <code>/abort</code> to terminate all runs.\n"
        msg += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        if not active_attacks:
            await update.message.reply_text(
                ...
                "└─────────────────────────────────────┘\n"
                "</pre>\n"
                "<i>No active strikes. Deploy one with /strike</i>",
                parse_mode='HTML'
            )
            return
        ...
        for aid, data in active_attacks.items():
            ...
            if remaining < 0:
                remaining = 0
            bar = progress_bar(elapsed, duration, width=20)
        ...
        msg += "└────────────────────────────────────────────────────────────┘\n"
        msg += "</pre>\n"
        msg += f"<i>🛑 Use /abort to terminate all missions.</i>"
        await update.message.reply_text(msg, parse_mode='HTML')
...
async def abort_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/abort – Terminate all active strikes."""
    try:
        user_id = update.effective_user.id
        if not can_attack(user_id):
            await update.message.reply_text("⛔ Access Denied", parse_mode='HTML')
    ...
    try:
        ...
        if can_attack(user_id):
            ...
            await update.message.reply_text(
                ...
                f"└──────────────────────────────────────────────┘\n"
                f"</pre>\n"
                f"<b>Quick Deploy:</b> <code>/strike &lt;ip&gt; &lt;port&gt; &lt;time&gt;</code>\n"
                ...
                for owner_id in owners.keys():
                    try:
                        await context.bot.send_message(
                            ...
                            f"👤 @{username}\n"
                            f"🆔 <code>{user_id}</code>\n"
                            f"Use: <code>/grant {user_id} 7</code>",
                            parse_mode='HTML'
                        )
                    except:
                        pass
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "<code>/strike &lt;ip&gt; &lt;port&gt; &lt;time&gt;</code> – Launch multi‑repo strike\n"
        "<code>/payload</code> – View live raid feed\n"
        "<code>/abort</code> – Emergency abort all raids\n\n"
        "<b>🔧 ADMIN PANEL</b>\n"
        "<code>/inject &lt;token&gt;</code> – Inject a GitHub token\n"
        "<code>/eject &lt;token&gt;</code> – Remove a token\n"
        "<code>/scan</code> – Health check with rate‑limit info\n"
        "<code>/purge confirm</code> – Wipe the vault\n"
        "<code>/vault</code> – List all tokens\n"
        "<code>/deploy</code> – Deploy the spider binary\n"
        "<code>/grant &lt;id&gt; &lt;days&gt;</code> – Grant access\n"
        "<code>/revoke &lt;id&gt;</code> – Revoke access\n"
        "<code>/list</code> – List approved users\n"
        "<code>/requests</code> – Pending requests\n"
        "<code>/announce &lt;msg&gt;</code> – Send announcement\n\n"
        "<b>ℹ️ UTILITY</b>\n"
...
async def matrix_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/matrix – Display a Matrix rain ASCII art."""
    matrix = """
┌─────────────────────────────────────────────┐
│  ░░▒▒▓▓██  ██▓▓▒▒░░  ░░▒▒▓▓██  ██▓▓▒▒░░  │
│  ██▓▓▒▒░░  ░░▒▒▓▓██  ██▓▓▒▒░░  ░░▒▒▓▓██  │
...
└─────────────────────────────────────────────┘
"""
    await update.message.reply_text(f"<code>{matrix}</code>\n\n<i>Welcome to the Matrix.</i>", parse_mode='HTML')

# ============================================================
# ===== ADMIN USER COMMANDS (Hacker Style) =====
# ============================================================
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "strike_help":
            ...
            await query.edit_message_text("📤 Use <code>/deploy</code>", parse_mode='HTML')
