...
MAX_REPOS_PER_ATTACK = int(os.environ.get("MAX_REPOS_PER_ATTACK", "5"))        # how many repos (tokens) to use simultaneously
...
# ============================================================
# ===== TOKEN MANAGER – RATE‑LIMIT AWARE =====
# ============================================================

class TokenManager:
    def __init__(self, tokens_list):
        self.tokens = tokens_list  # list of dicts with token, username, repo, etc.
        self.in_use = {}           # token -> count of active attacks using it
        self.ratelimit_cache = {}  # token -> remaining requests
    ...
    def health_check(self):
        """Remove invalid tokens and update rate‑limit info."""
    ...
    def get_healthy_tokens(self, max_count, exclude=[]):
        """Return up to max_count tokens that are not rate‑limited and have remaining > threshold."""
        healthy = []
        # sort by remaining (higher is better) but also randomise to spread load
        ...
        # also consider current usage (in_use) – prefer less used
        for td in candidates:
            ...
            if remaining < TOKEN_RATE_LIMIT_THRESHOLD:
                logger.info(f"⏳ Token @{td['username']} low on rate‑limit ({remaining}) – skipping")
                continue
            # check if in_use count is low (avoid overloading)
            if self.in_use.get(token, 0) < 2:  # allow up to 2 concurrent attacks per token
                healthy.append(td)
                if len(healthy) >= max_count:
                    break
        # if not enough, include tokens with higher usage but still healthy
        if len(healthy) < max_count:
            for td in candidates:
...
# ============================================================
# ===== BINARY UPLOAD (unchanged but uses token manager) =====
# ============================================================
...
# ============================================================
# ===== PROFESSIONAL GENZ ATTACK COMMAND – MULTI‑REPO =====
# ============================================================
    ...
    try:
        ...
        for token_data in healthy_tokens:
            ...
            try:
                ...
                # Build YML with dynamic runners and threads
...
"""
        ...
        # Build professional GENZ output
        ...
        message = (
            ...
            f"│  ⚙️ RUNNERS/REPO   │  {RUNNERS_PER_ATTACK} × {THREADS_PER_RUNNER} threads               │\n"
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        for aid, data in active_attacks.items():
            ...
            # get duration from first target (all same)
            remaining = 0
            if targets:
                # we store duration in each target? Actually we don't store time per target; we can use the attack timer.
                # better: store duration in attack metadata
                # we'll add duration to data when creating
                # quick fix: read from active_attacks[aid]['duration'] if exists
...
# ============================================================
# ===== OTHER COMMANDS (mostly unchanged) =====
# ============================================================
