...
async def gunshot_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # We need to re-deploy with the same parameters.
    # We'll call a helper function that does the deployment and sends a new message.
    # Since we have the same logic as strike_cmd, we can extract the core into a function.
    # I'll inline the deployment logic here (same as strike_cmd but without command parsing).
    # We'll re-use the same token selection and deployment.
    try:
        token_manager.health_check()
        if not github_tokens:
...
def main():
    try:
        ...
        # Core commands (only these, no aliases)
